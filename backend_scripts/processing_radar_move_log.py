...
- Creates a database of the basic scan info for use by the
    `simulation_radar.py` script during the simulation
...
for i in range(len(glob_list)):
    ...
    scan_datetime = datetime(year, month, day, hour, minute, second,
                             tzinfo=pytz.UTC)
...
for scan in scan_list:
    print('Moving {} {}'.format(
        scan['site'],
        scan['datetime'].strftime(std_fmt)
    ))
...
rad_cur.execute(
    "CREATE TABLE scans (time datetime, site char, file char, munged tinyint)"
)
rad_con.commit()
