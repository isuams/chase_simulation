...
def process_warning_text(warning, timings):
	...
	# Also, since we need it later, get the starting timestamp in our standard format
	...
	if match:
	    ...
	    for time_obj in (arc_local_time, cur_cdt_time):
	        ...
	        str_to_swap.append(" ".join((swap_time, swap_zone, swap_weekday, swap_month, swap_day, swap_year)))

	    warning = warning.replace(*str_to_swap)
	    
	    # Now, do likewise (with some magic) for all the '622 PM CDT' after the end of that
