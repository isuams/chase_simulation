	...
	while running:
		...
		if len(warnings_to_release) > 0:
			...
			for warning_row in warnings_to_release:
				...
				warning, warning_cur_time = process_warning_text(warning, timings=settings)

				# Then, determine the proper warning txt file, and append this warning
				warning_file = warning_dir + warning_cur_time.strftime('warnings_%Y%m%d_%H.txt')
