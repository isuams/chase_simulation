...
# Get the user input
arc_start_time = parser.parse(
    input('When does the archived case start?\n\t(use UTC) : ')
)
...
print("\t{} (having {} of radar data)".format(
    arc_start_time.strftime(std_fmt),
    arc_td_str
))
print("\tstarting at current time")
print("\t{} (having {} of radar data)?".format(
    cur_start_time.strftime(std_fmt),
    cur_td_str
))
print()
...
To do that, run
    simulation_radar.py (radar scans) and
    simulation_warning.py (warning text).
To reset, run
    simulation_reset.py,
